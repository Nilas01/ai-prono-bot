...
# Liste pour détecter les pays et clubs
...
async def analyser_le_match(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # =======================================================
    # 🧠 SYSTÈME DE SIMULATION LOGIQUE FIXE (INDÉPENDANT DE L'ORDRE)
    # =======================================================
    # On crée une clé unique basée sur le nom des deux équipes triées par ordre alphabétique.
    # Ainsi, "Portugal-Congo" et "Congo-Portugal" donneront EXACTEMENT la même clé et le même résultat.
    cle_unique = "".join(sorted([equipe_dom.lower(), equipe_ext.lower()]))
    random.seed(cle_unique) # Force le hasard à être identique pour ce duel

    # Simulation de la force des équipes (Exemple: le Portugal ou les grands clubs ont souvent l'avantage)
    ...
    # Calcul des scores basé sur la force fixe
    ...
    if score_dom_calcule < 0: score_dom_calcule = 0
    if score_ext_calcule < 0: score_ext_calcule = 0

    # Détermination logique de la recommandation
    ...
    # Génération des détails chiffrés demandés
    b_1ere = random.randint(0, min(score_dom_calcule + score_ext_calcule, 2))
    b_2eme = (score_dom_calcule + score_ext_calcule) - b_1ere
    if b_2eme < 0: b_2eme = 0
    ...
    if "Football" in sport or "Handball" in sport or "Rugby" in sport:
        ...
        q1, q2, q3, q4 = random.randint(40, 50), random.randint(43, 53), random.randint(41, 51), random.randint(45, 55)
        details_chiffres = (
        ...
        s1, s2 = random.randint(6, 7), random.randint(4, 6)
        details_chiffres = (
    ...
    random.seed() # Réinitialise le générateur pour les autres utilisateurs
    ...
    if "Avant-Match" in moment:
        rapport = (
            ...
            "▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬\n\n"
            f"📈 **STATUT ET COMPORTEMENT DES BLOCS**\n"
            f"• {drapeau_dom} **{equipe_dom}** : Analyse globale basée sur l'historique de l'effectif.\n"
            f"• {drapeau_ext} **{equipe_ext}** : Évaluation tactique du bloc adverse terminée.\n\n"
            f"{details_chiffres}\n"
        ...
        rapport = (
            ...
            f"🟨 **FAUTES & DISCIPLINE** : Match engagé avec des arrêts de jeu répétés.\n\n"
            f"                                      _prédictions fait par le zehi_"
        )
...
def main():
    if not TELEGRAM_TOKEN or TELEGRAM_TOKEN == "8652808582:AAFS5nG7xFxBpjI8aPMW5Y7Km7ME3Db-rOY"
        print("Erreur : Remplace TON_TOKEN_ICI à la ligne 15 !")
        return
